...
{
    int colorIndex = 0;
    while (digitalRead(button1) && digitalRead(button2) && !IrReceiver.decode())
    { // cette boucle continue tant que les deux boutons ne sont pas appuyés et que aucune commande IR n'est reçue
        for (int i = 0; i < 9; i++)
        {
            for (int j = 0; j < 16; j++)
            {
...
void chronometre()
{
    ...
    int delayTime = 60000;       // 1 minute en ms
    int step = 256 / NUM_PIXELS; // nombre d'étapes pour passer de blanc à bleu
    int currentStep = 0;
    ...
    while (millis() - startTime < delayTime)
    {
        for (int i = 0; i < NUM_PIXELS; i++)
        {
            NeoPixel.setPixelColor(i, NeoPixel.Color(currentStep, currentStep, 255));
        }
        NeoPixel.show();
        currentStep += step;
        delay(10);
